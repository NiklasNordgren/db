...
BEGIN
    ...
    IF CHAR_LENGTH(NEW.CourseCode) < 6 THEN
		SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Course code must be at least six alphanumerical characters long';
    END IF;
    IF NOT REGEXP_LIKE(NEW.CourseCode, '(^([A-ZÅÄÖ]{3}[0-9]{3})([A-ZÅÄÖ])?)|(^([A-ZÅÄÖ]{2}[A-ZÅÄÖ0-9]{4}))', 'c') THEN
		SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Course code must start with two uppercase letters followed by four alphanumerical characters (letters & numbers) followed by an optional letter';
    END IF;
